...
Taken from https://github.com/python-hyper/hyper-h2/blob/master/examples/asyncio/wsgi-server.py
...
if __name__ == '__main__':
    ...
    # Generate a self-signed temporary cert and key. The files will
    # exist as long as the file objects are open.
    SSL_CERT_FILE = tempfile.NamedTemporaryFile()
    SSL_KEY_FILE = tempfile.NamedTemporaryFile()
    cid = str(uuid.uuid4())
    print('Generating certificate...')
    subprocess.call(
        shlex.split(
            'openssl req -new -newkey rsa:2048 -sha256 -days 30 -nodes -x509 '
            '-batch -subj "/C=US/ST=CA/L=SF/O=%s/CN=%s.com" '
            '-keyout %s -out %s' % (
                cid, cid, SSL_KEY_FILE.name, SSL_CERT_FILE.name)
        )
    )
    ...
    ssl_context.load_cert_chain(SSL_CERT_FILE.name, keyfile=SSL_KEY_FILE.name)
    ssl_context.set_alpn_protocols(["h2"])
    ssl_context.set_npn_protocols(["h2"])

    # APNS-specific setup

    ssl_context.set_ciphers(':'.join(APNS_CIPHERS))

    # Do the asyncio bits
    loop = asyncio.get_event_loop()

    # Each client connection will create a new protocol instance
    coro = loop.create_server(H2Protocol, '0.0.0.0', FCM_PORT, ssl=ssl_context)
